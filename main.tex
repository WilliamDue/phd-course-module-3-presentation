...
\begin{frame}\frametitle{NFA Simulation}
  \begin{center}
    \begin{tikzpicture}[shorten >=1pt, node distance=2.6cm, on grid, auto, >=stealth', baseline]
      ...
      \draw[->,edgeeps] (q0) -- node[left] {$\epsilon$} (q4);
    \end{tikzpicture}

    \vspace{10pt}
    % Input string animation
    \only<1-2,4-8>{\Huge\bfseries\color{gray}ab}
    \only<3,9>{\Huge\bfseries\color{\darkpalepurple}a\color{gray}b}
    \only<10>{\Huge\bfseries\color{gray}a\color{\darkpalepurple}b}
    \only<11>{\Huge\bfseries\color{\darkpalepurple}ab}
    \vspace{10pt}
    % Acceptance message
    \only<11>{\Large\bfseries\color{\darkpalepurple}Accepted!}
  \end{center}
\end{frame}
...
\begin{frame}\frametitle{DFA}
  \begin{itemize}
    ...
    \item No $\epsilon$ transitions allowed.
